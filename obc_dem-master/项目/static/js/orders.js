...
function ax() {
    axios({
        ...
        method: 'post',
        url: 'http://localhost:3066/orders',
        // data: Qs.stringify({
        //     username: ipt.value
        // })
    }).then((data) => {
        for (let i = 0; i < data.data.length; i++) {
            ...
            divs.innerHTML = img.innerHTML
            divs.className = 'img shopbox'
            shop.appendChild(divs)
            let imge = document.querySelectorAll('.shopbox img')
            let js = document.querySelectorAll('.shopbox .js')
            let money = document.querySelectorAll('.shopbox .money1')
            let mach = document.querySelectorAll('.shopbox .mach')
            let zj = document.querySelectorAll('.A1')
            imge[i].src = data.data[i].photo
            js[i].innerHTML = data.data[i].wenben
            money[i].innerHTML = data.data[i].money
            mach[i].innerHTML = data.data[i].mach
            zj[0].innerHTML = data.data[0].more
            let divv = document.querySelectorAll('.shopbox')
            divv[i].setAttribute('id', data.data[i].id)
            // 传入id
            ...
            btn.onclick = function () {
                ...
                yes.onclick = function () {
                    for (let i = 0; i < data.data.length; i++) {
                        id = data.data[i].id
                        zj = data.data[i].money * data.data[i].mach
                        let tj = 1
                        let cg = 1
                        axios({
                            headers: {
...
let xian = document.querySelector('.qu')
// let gs = document.querySelector('.gs')
// let gshi = document.querySelector('.gshi')
// let gx = document.querySelector('.gx')
let xiu = document.querySelector('.masks span')
...
xiu.onclick = function () {
    var qu_index = qus.selectedIndex;
    let gs = areas.children[area_index].innerHTML
    let gshi = citys.children[citys_index].innerHTML
    let gx = qus.children[qu_index].innerHTML
    sheng.innerHTML = gs
    shi.innerHTML = gshi
    xian.innerHTML = gx
    ...
    let xx = document.querySelector('.xiangxi')
    xx.innerHTML = textarea.value
}
mb.onclick = function () {
    mb.style.display = 'none'
    masks.style.visibility = 'hidden'
